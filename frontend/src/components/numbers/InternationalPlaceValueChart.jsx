...
import { groupForColumnIndex, GROUP_COLORS } from '../../utils/internationalGroupColors.js'
...
// International comma grouping: groups of 3 digits — add a visual gap before these column indices.
const GROUP_START_INDICES = new Set([3, 6])
...
function InternationalPlaceValueChart({ number = 74532618 }) {
            ...
            className={`flex flex-col items-center gap-1 shrink-0 rounded-xl px-1.5 sm:px-2 py-2 sm:py-3 min-w-[46px] sm:min-w-[64px] transition-colors
              ...
              ${
                selectedIndex === i
                  ...
                  : GROUP_COLORS[groupForColumnIndex(i)].chip
              }`}
          >
        ...
        >
          ...
          <p className="text-projector-sm text-ink">
            ...
            <span className="font-bold text-primary">
              {selected.placeValue.toLocaleString('en-US')}
            </span>
          </p>
          <p className="text-base text-slate-500 pt-2">
            ...
            × {PLACE_VALUES[selectedIndex].toLocaleString('en-US')} ={' '}
            {selected.placeValue.toLocaleString('en-US')}.
          </p>
        </motion.div>
      )}
...
export default InternationalPlaceValueChart
