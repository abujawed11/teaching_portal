...
import { numberToInternationalWords } from '../../utils/numberToInternationalWords.js'
...
function validate(value) {
  ...
  if (parsed < MIN || parsed > MAX) return `Enter a number between ${MIN} and ${MAX.toLocaleString('en-US')}.`
  return null
}

function InternationalNumberConverter() {
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full">
      ...
      <div className="bg-slate-50 rounded-2xl px-8 py-6 text-center space-y-3 max-w-2xl">
        ...
        <p className="text-projector-sm text-ink">{numberToInternationalWords(number)}</p>
      </div>
    </div>
  )
}

export default InternationalNumberConverter
