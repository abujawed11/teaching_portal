...
### Working agreement: Note box vs. Analogy box — don't conflate them

Per user catch (Class 6 Science `what-is-biodiversity` intro step): the `analogy` prop on `text`/`intro` steps (amber box, "Think of it like this:") is for genuine comparisons only — relating a new idea to something familiar (e.g. "science is like a jigsaw puzzle," "the soil is like a sponge"). It was being misused for a plain but important statement that wasn't a comparison at all ("this variety of plants and animals has a name — biodiversity"), which read oddly under a "Think of it like this" label.

Fixed by adding a second, visually distinct **Note** box: a new `note` prop on `text`/`intro` steps (sky-blue box, Info icon, "Note:" label), rendered by `ExplanationPanel`/`LessonStepRenderer` alongside `analogy`. Use `note` for a plain but important/emphasis-worthy statement (a definition lead-in, a key rule, something worth visually setting apart) that is NOT a metaphor or comparison. Use `analogy` only when there's an actual "it's like X" comparison. Don't stretch one to cover the other. First applied in `whatIsBiodiversity.js`.

### Working agreement: "Do you know?" / general-knowledge facts as question-style reveals

Per user instruction (raised while building Class 5 Science Module 1 — Freshwater vs. Saltwater): whenever a lesson includes a "Do you know?"-style general-knowledge fact (trivia asides like the book's info boxes — e.g. Gujarat's salt pans, Wular Lake, the Luni River, the Statue of Unity), it should be presented as a `question` step (fact rephrased as a question) with the fact itself as the `answer` and any extra context as the `explanation` — not as a plain `text` step. Reason: this is for the teacher's convenience — if a curious student asks about a fact mid-class, the teacher has a ready-made question-and-answer already built into the lesson, consistent with how practice questions already work (question → reveal → explanation), rather than a fact just stated outright with nothing to "ask" about it. Applies to all subjects going forward, not just Science. First applied to `freshwaterVsSaltwater.js`'s salt-pans fact.
...
**Decision: image vs. SVG, condition-based (added during Class 5 Science Module 1 build)**: per user instruction, whether a lesson visual is a real photo or an original SVG/CSS illustration is decided per-visual, not with one blanket rule:
- **Real-world recognition content** (e.g. "what does an actual pond/glacier/mountain lake look like" identification activities) → use a real photo. Claude references the image path in code (e.g. `/images/science/water-essence-of-life/pond.jpg`) and tells the user exactly which filename(s) to drop into the matching `frontend/public/images/...` folder — same convention already used for the Computer course (`public/images/computer/foundations/*.jpg`).
- **Conceptual/decorative content** (a diagram, a hook illustration, a proportion demo) → build an original SVG/CSS component in code instead (e.g. `WaterDropIllustration.jsx`, the earlier `PizzaFractionIllustration.jsx`). No external asset needed.

A Canva plugin/MCP connector was connected specifically to try generating original images automatically (removing the need for the user to source photos by hand), but `generate-design` failed with "Design generation is not enabled in your team" — an account/plan restriction on the Canva side, not fixable from within Claude Code. There is also no stock-photo search tool exposed via this Canva MCP connection (`get-assets` only fetches metadata for images by ID, it doesn't search). So for now, photo-needing visuals still require the user to manually source and add images to the public folder; this may be revisited if Canva's AI design generation is enabled on the account later.
...
**Module 6 (✅ Done)**: `src/components/science/TraitRevealTable.jsx` (land-vs-water animal traits, staged reveal — breathing/moving/body covering), `FreshwaterCreatureCards.jsx` (the book's 8-organism Information Card as photo cards — dragonfly, water scorpion, pond heron, freshwater turtle, water snake, reeds, lotus/water lily, water hyacinth — **needs 8 photos added** to `public/images/science/water-essence-of-life/`: `dragonfly.jpg`, `water-scorpion.jpg`, `pond-heron.jpg`, `freshwater-turtle.jpg`, `water-snake.jpg`, `reeds.jpg`, `lotus-water-lily.jpg`, `water-hyacinth.jpg`), `WaxyLeafDemo.jsx` (Activity 9 — plain paper vs. wax-crayon paper water-drop comparison, original CSS/motion, no photo needed), `FoodChainBuilder.jsx` (Activity 10 — builds a pond food chain step by step, then shows the ripple effect of removing small fish), `src/data/lifeInFreshwaterHabitats.js`, `quizzes['life-in-freshwater-habitats']` (10 questions). Verified with `npm run build` — no errors.
